...
export function AsyncJobsScreen() {
  ...
  // ─── recorder state (we use the bare ExpoRecorder, not useRecorder,
  ...
  const recRef = useRef<ExpoRecorder | null>(null);
  ...
  const ensureRecorder = () => {
    ...
    // The `Audio` module's enum-typed RecordingOptions don't match the
    // SDK's narrower `ExpoAudioModule` shape. The cast mirrors what
    // `useRecorder({ audio: Audio })` does internally.
    const r = new ExpoRecorder(Audio as any, {
      onStateChange: setRecState,
      onLevel: setLevel,
      onStatus: (st) => setDurationSec(st.durationMillis / 1000),
    });
    recRef.current = r;
    return r;
  };
